...
int main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::OnOffApplication::PacketSize", StringValue ("250kb/s"));
...
// Convert to time object
  ...
  Config::SetDefault ("ns3::WifiRemoteStationManager::FragmentationThreshold", StringValue ("2500"));
...
// Here, we will create n nodes.
  ...
  NodeContainer allNodes = NodeContainer (serverNode, clientNodes, cn, cn_extra, cn_extra_2);
...
//normally wouldn't need a loop here but the server IP address is different
  ...
  // AnimationInterface anim ("without_attack.xml"); // Mandatory
...
for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
          ...
          std::cout << "Delay : " << (i->second.delaySum.GetSeconds() / i->second.txPackets) ;
          std::cout << "\n";
    }
        std::cout << "\n\n\n ######################## Final Conclusion ########################" << "\n\n";
        ...
        std::cout << "  Packet Drop Ratio: " <<(double)((DropPacketsum/txPacketsum) *100 ) << "% \n";
        //std::cout << "  Packets Delivery Ratio: " << ((rxPacketsum * 100) /txPacketsum) << "%" << "          \n" << "  Packets Lost Ratio: " << (double)((LostPacketsum * 100) /txPacketsum) << "%" << "\n";
        std::cout << "  All Delay: " << Delaysum / txPacketsum << "\n";
        std::cout << "  Average END-TO-END delay: " << ((Delaysum / txPacketsum) / rxPacketsum) * 2990000 << "\n" ;
        std::cout << "  Throughput: " << (rxBytesum * 8.0 / 10.0 / 1024 / 1024)*100  << " bits/s\n";
