...
int main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::OnOffApplication::PacketSize", StringValue ("2500kb/s"));
...
for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
          ...
          if( (i->second.txPackets - i->second.rxPackets) != 0){
            if( (i->second.txPackets - i->second.rxPackets) > packet_loss_threshold ){
              if( (i->second.delaySum.GetSeconds() / i->second.txPackets) < delay_threshold ){
        ...
        std::cout << "  Average END-TO-END delay: " << ((Delaysum / txPacketsum) / rxPacketsum) * 1000000  << "\n" ;
